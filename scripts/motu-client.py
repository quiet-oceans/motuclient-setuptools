...
def load_options():
    ...
    parser = optparse.OptionParser(version=get_client_artefact() + ' v' +
                                   get_client_version())
    ...
    parser.add_option('--depth-min', '-z',
                      type='string',
                      help="The min depth "
                      "(float in the interval [0 ; 2e31] or string 'Surface')")
    ...
    parser.add_option('--sync-mode', '-S',
                      help="Sets the download mode to synchronous "
                      "(not recommended)",
                      action='store_true',
                      dest='sync')
    ...
    parser.add_option('--block-size',
                      type='int',
                      help="The block used to download file "
                      "(integer expressing bytes)",
                      default="65536")
    ...
    for option in parser.option_list:
        if (option.dest is not None) and conf_parser.has_option(SECTION,
            ...
            if option.dest == "variable":
                default_variables.append(conf_parser.get(SECTION,
                                                         option.dest))
...
def check_version():
    ...
    if ((cur_version[0] > REQUIRED_VERSION[0] or
        ...
        raise Exception("This tool uses python 2.7 or greater. You version "
