...
from motu import utils_unit
from motu import utils_stream
from motu import utils_http
from motu import utils_messages
from motu import utils_cas
from motu import utils_collection
from motu import stop_watch
...
def check_options(_options):
    ...
    if (((_options.proxy_server is not None) and
         (len(_options.proxy_server) != 0))):
        _options.proxy = True
        # check that proxy server is a valid url
        url = _options.proxy_server
        p = re.compile('^(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)'
                       '(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?')
        m = p.match(url)
        ...
        # check that if proxy-user is defined then proxy-pwd shall be also,
        # and reciprocally.
        if (((_options.proxy_user is not None) !=
             (_options.proxy_pwd is not None))):
            raise Exception(
                utils_messages.get_external_messages()[
...
def execute_request(_options):
    ...
    * Service identifier to use for retrieving dataset
      - service_id: 'http://purl.org/myocean/ontology/service/'
                    'database#yourduname'
    ...
    * Temporal extraction parameters, as a datetime instance or a string
    (format: '%Y-%m-%d %H:%M:%S')
